  ...
  if (action === 'add' && participants) {
    for (const participant of participants) {
      if (/^(2|33|44|51)/.test(participant)) {
        await conn.groupParticipantsUpdate(jid, [participant], 'remove');
      }
    }
  }
})
...
import { WA_DEFAULT_EPHEMERAL, waChatKey, mentionedJid } from '@adiwajshing/baileys/build/WAConnection/Constants';
import fs from 'fs';
