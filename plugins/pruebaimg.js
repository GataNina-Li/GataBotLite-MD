...
const handler = async (m, { conn, text }) => {
  ...
  const imageWidth = baseWidth + Math.floor(formattedText.length / 50) * 75
  const imageHeight = Math.max(baseHeight, totalTextHeight + 100 + Math.floor(formattedText.length / 50) * 19)
