...
try {
const response = await await fetch(`https://api.alyachan.dev/api/yta?url=${userVideoData.url}&apikey=Gata-Dios`) //fetch(APIs.alyachan.url + `yta?url=${userVideoData.url}&apikey=${APIs.skizo.key}`)
...
await conn.sendMessage(m.chat, { audio: { url: downloadUrl }, mimetype: 'audio/mpeg' }, { quoted: gata.resp });
} catch {
try {
