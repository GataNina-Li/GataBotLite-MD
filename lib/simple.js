...
import chalk from 'chalk'https://github.com/GataNina-Li/GataBotLite-MD/edit/master/lib/simple.js
    ...
    */
    ...
    async value(jid, data, quoted, options) {
        ...
        for (let [number, name, isi, isi1, isi2, isi3, isi4, isi5] of data) {
            ...
            let njid = number + '@s.whatsapp.net'
            let biz = await conn.getBusinessProfile(njid).catch(_ => null) || {};
            let imageData = biz.img ? await conn.downloadProfilePic(njid, true) : null;
            if (!imageData) {
                imageData = await fs.promises.readFile(img)
            }
            // N:;${name.replace(/\n/g, '\\n').split(' ').reverse().join(';')};;;
            let vcard = `
BEGIN:VCARD
...
PHOTO;TYPE=JPEG:${imageData.toString('base64')}
END:VCARD`.trim()
            contacts.push({ vcard, displayName: name })
        }
        return await conn.sendMessage(jid, {
